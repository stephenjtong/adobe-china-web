...
def asteriskFile(filename,baseurl='www.stage.adobe.com'):
	...
	content=re.sub(r'(/.*?\.ssi)',r'/'+geoCode+r'\1',content) #将所有的ssi文件链接改为本地
	...
	content=re.sub(r'href="(/[^g]./.*?)"', asteriskLink,content,re.S) #检查以GEO code开头的链接，如果有坏链则去掉GEO code
	content=re.sub(r'<area.*?href="(/[^g]./.*?)"', asteriskLink,content) #检查<area .... />中的链接
	content=re.sub(r'([^ ]) *?(</a>)',r'\1\2',content)#去掉</a>前多余的空格	
	
	content=re.sub(r'(<a[^<>]*>.*?)(</a>)',r'\1*\2',content) #全部URL加星号
	content=re.sub(r'(/[^g]./[^<>]*>[^<>]*)\*(</a>)',r'\1\2',content) #有GEO的链接星号去掉
	content=re.sub(r'(<a.*?>.*?<img.*?)\*(</a>)',r'\1\2',content) #图片上链接的星号去掉
	content=re.sub(r'(<a[^<>]*button[^<>]*>[^<>]*)\*(</a>)',r'\1\2',content) #button的星号去掉
	content=re.sub(r'(<a[^<>]*href="#[^<>]*>[^<>]*)\*(</a>)',r'\1\2',content) #带锚点的链接的星号去掉
	
	content=re.sub(r'(href="/go/[^?]*?)"',r'\1_'+geoCode+r'"',content) #将所有的gourl加上_[geoCode]，忽略带promoid的
